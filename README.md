...
面向中英双语通用企业知识库的完整 RAG 项目。阶段 1（数据库、文件存储与可靠异步任务）已完成，下一步进入阶段 2 的 Parser Router 与 Canonical Model。

完整实施方案见 [docs/IMPLEMENTATION_PLAN.md](docs/IMPLEMENTATION_PLAN.md)。

## 当前工程组成

- `backend/`：FastAPI、SQLAlchemy/Alembic、Celery Worker、配置、日志与测试。
...
上传成功立即返回不可变文档版本和持久化 Job。文件完成安全校验与原子落盘后，Job 会停在 `parsing`，由阶段 2 接入解析器继续推进。
